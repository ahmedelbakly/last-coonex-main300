...
import { leadsSourceData, rows } from "src/fileData/cmsLeadsData";
...
const AddNewLeads = () => {
  const auth = useAuth();
  auth.setPages(" CRM Edit Leads");
  const [overLaySuccess, setOverLaySuccess] = useState(false);
  const [emptyItem, setEmptyItem] = useState({});

  //** FILTER LEADS BY LEAD ID */
  useEffect(() => {
    if (auth.leadId) {
      const lead = rows.find((lead) => lead.id === auth.leadId);
      setAddLeadData(lead);
    }
  }, [auth.leadId]);
